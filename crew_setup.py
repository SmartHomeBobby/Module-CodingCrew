...
from stakeholder_tool import AskStakeholderTool
...
def create_coding_crew(
    ...
    # 3. Setup the tools
    stakeholder_tool = AskStakeholderTool(
        mqtt_handler=mqtt,
        request_topic=decision_request_topic
    )
    ...
    dev_tools = [stakeholder_tool, execution_tool]
    git_tools = [stakeholder_tool, execution_tool, github_tool, git_commit_push]
    ...
    planning_task = Task(
        description=f"Analyze the project goal: '{project_goal}'. Consider all these technical details: '{technical_details}'. "
                    ...
                    "Ask the stakeholder for any required API keys or missing product requirements regarding user onboarding or features.",
    ...
    # 6. Assemble the Crew
    crew = Crew(
        agents=[software_architect, senior_developer, quality_assurance, data_privacy_officer],
